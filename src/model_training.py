...
class ModelRetraining:
    ...
    def training(self):        
        ...
        for anti_id, anti_name in self.get_antimicrobial_ans().items():
            ...
            X_test_dummies = self.get_dummies_dataframe_columns(X_train_dummies, pd.get_dummies(X_test))
    ...
    def test_by_case(self, version: int):
        anti_ans = ["ans_" + anti_name for anti_name in self.get_antimicrobial_ans().values()] # antimicrobial answer startswith "ans_"
        ...
        models = [["ans_" + row[0], joblib.load(row[1]), eval(row[2])] for row in self.get_model(version).values] # load model
        df_predict = pd.DataFrame()
        
        for model in models:
            df_schema = pd.DataFrame(columns=model[2]) # create schema
            X_dummies = self.get_dummies_dataframe_columns(df_schema, pd.get_dummies(X_bycase)) # one-hot
    ...
    def compare_model_by_f1(self, X_test, y_test, model_path_old: str, schema_old: list, f1_new: float):
        ...
        X_test_dummies = self.get_dummies_dataframe_columns(df_schema, pd.get_dummies(X_test))
        f1_old = self.evaluation(X_test_dummies, y_test, joblib.load(model_path_old))["f1"]
        if f1_new > f1_old:
            performance = "better"
        elif f1_new < f1_old:
            performance = "worse"
    ...
    def update_model_current_version(self):
        ...
        performance_model = self.get_performance_model(version=self.lastest_version())
        for perf in performance_model:
            if perf["performance"] == "better":
                mgm_id = self.get_model_group_model_id(mg_id=current_model_group_id, anti_id=perf["anti_id"])
                with self.conn.connect() as con:
                    query = sqlalchemy.text(
                        """
    ...
    def get_antimicrobial_ans(self) -> Dict:
        ...
        ans_name = pd.read_sql_query(query_ans, self.conn, params={"v_id": self.vitek_id})
        anti_id_range = [
            np.arange(1, 12),  # GN
            np.arange(12, 23),  # GP
        ]
        anti_ans = {row[0]: row[1] for row in ans_name[ans_name['id'].isin(anti_id_range[self.vitek_id-1])].values}
        return anti_ans
    ...
    def get_model(self, version: int):
        query_model = sqlalchemy.text("""SELECT ans.id, ans.name, model_path, schema
            ...
            ORDER BY ans.name""")
        model = pd.read_sql_query(query_model, self.conn, params={"version": version, "v_id": self.vitek_id})
    ...
    def lastest_version(self):
        query = sqlalchemy.text(
            ...
            """)
        lastest_version = pd.read_sql_query(query, self.conn, params={"vitek_id": self.vitek_id}).values[0][0]
    ...
    def get_performance_model(self, version: int) -> Dict:
        query = sqlalchemy.text(
            ...
            """)
        performance = pd.read_sql_query(query, self.conn, params={"v_id": self.vitek_id, "version": version})
    ...
    def get_current_model_group_id(self, version: int):
        query = sqlalchemy.text(
            ...
            WHERE vitek_id = :v_id AND version = :version
            """)
        model_group_id = pd.read_sql_query(query, self.conn, params={"v_id": self.vitek_id, "version": version}).values[0][0]
    ...
    def get_model_group_model_id(self, mg_id: int, anti_id: int):
        query = sqlalchemy.text(
            ...
            """)
        mgm_id = pd.read_sql_query(query, self.conn, params={"mg_id": mg_id, "anti_id": anti_id}).values[0][0]
        return int(mgm_id)
