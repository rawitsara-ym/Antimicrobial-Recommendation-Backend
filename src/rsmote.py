...
class RSmote:
    ...
    def over_sampling(self):
        ...
        num_maj = number_maj(
            self.train[:, 1:], self.train_less[:, 1:], self.tp_less, self.train[:, 0])
        ...
        # Control the maximum density range at 100
        density = list(map(lambda x: min(100, x), density))

        # The density is sorted below, and the minority samples are also sorted in order of density.
        density_sorted = sorted(
            range(len(density)), key=lambda a: density[a], reverse=True)  # sorted
        data_resorted = []
        density_sorted_data = []
        num_sorted = []
        ...
        kk = self.k
        # the number of samples need to synthesize
        diff = len(self.train_more) - length_less
        ...
        for i in range(flag):
            ...
            # if len(self.train_less)<k,set k =len(self.train_less)
            self.k = min(len(self.train_less) - 1, kk)
            ...
            neighbors = NearestNeighbors(
                n_neighbors=self.k + 1).fit(self.train_less[:, 1:])
            nn_array = neighbors.kneighbors(
                self.train_less[:, 1:], return_distance=False)

            self.synthetic = np.zeros((number_synthetic, self.n_attrs - 1))
            for p in range(self.train_less.shape[0]):
                self._populate(p, nn_array[p][1:],
                               number_synthetic, N, maj_num_ab)

            label_synthetic = np.array(
                [self.tp_less] * number_synthetic).reshape((number_synthetic, 1))
    ...
    def _populate(self, index, nnarray, number_synthetic, N, maj_num_ab):
        ...
        for j in range(turn):
            if self.new_index < number_synthetic:
                ...
                dif = self.train_less[nnarray[nn], 1:] - \
                    self.train_less[index, 1:]
                gap = random.random()
                # gap = np.random.rand(attribute)
                self.synthetic[self.new_index] = self.train_less[index,
                                                                 1:] + gap * dif
                self.new_index += 1
            else:
                break
...
class RSmoteKClasses:
    ...
    def fit_resample(self, X, y):
        data = np.hstack((y.values.reshape((len(y), 1)), X))
        counter = Counter(y)
        ...
        for k, v in counter.items():
            ...
            if data_new.shape[0] == 0:
                ...
                data_new = np.vstack(
                    (data_new, data_r_smote[data_r_smote[:, 0] != max_class_label, :]))

        X_resampled, y_resampled = data_new[:, 1:], data_new[:, 0]

        # ADDED BY YM
        y_resampled = pd.Series(y_resampled).astype('bool')
